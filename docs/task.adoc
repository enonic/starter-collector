...
Additionally, the task must be explicitly declared in the `collectors.json` file.
...
As you can see from the above file, the task name is listed per collector definition. This means two different collectors could potentially use the same task.

NOTE: Remember to update collectors.json if you change the name of your task.
